...
- 每个发布小节必须先写 `#### Summary`：1-4 条普通、用户能直接看懂的短 bullet。自动生成的 Sparkle 更新窗口会把这些摘要渲染成视觉卡片。这里要按普通用户视角写，只说更新后用起来哪里更好，不写实现、测试、CI、PR 或发版链路细节。
...
- Claude 凭据处理更简单，5 小时配额重置后也会保留上次百分比而不是变成空白
- 同一个应用现在也提供 CodexMonitor 版本——功能完全一致，仅名称不同，并有独立的更新通道
- 应用窗口和菜单栏面板重新使用系统标准的窗口方式打开，菜单栏读数只显示有实时数字的工具

### 新增
- **产品手册。** 新的中文指南配合截图说明初始引导、菜单栏弹窗、Dashboard、History、Sessions、Settings、更新和卸载流程。
- **CodexMonitor 版本。** 每次发布现在会同时产出 CodexMonitor 品牌的安装包，并使用独立更新源；无论使用哪个名称，现有安装的数据与设置都保持不变。

### 变更
- **Claude 凭据设置。** 高级设置现在默认使用自动 Claude 凭据刷新，不再把仅文件/钥匙串选择器作为常规选项；只有已保存的仅文件模式可能停止实时配额刷新时，才显示恢复按钮。
- **SwiftUI 窗口场景。** 仪表盘、设置、初始引导和菜单栏帮助窗口重新成为 SwiftUI `Window` 场景，通过轻量的 `WindowRouter` 桥接打开；这替换了 0.2.31 引入的 AppKit 窗口管理器。
- **系统菜单栏弹窗。** 菜单栏弹窗重新使用 `NSPopover`，定位和全屏行为交给系统处理，而不是手动摆放的无边框窗口。
- **菜单栏读数隐藏空 provider。** 状态栏文本读数现在会省略没有实时配额数字的 provider，而不是显示 `-- · --` 占位或回退到仪表盘配额快照。
- **真实数据 QA 默认值。** `qa/prepare-computer-use-real-data.sh` 重新应用确定性 QA 默认设置，不再复制已安装应用的 UserDefaults，并会等待 Developer Mode 日志出现设置遍历和快照事件后才报告成功。
- **单 job CI。** tests workflow 在每次 push 和 PR 上都运行静态 QA 门禁加 release 构建冒烟检查，移除了按变更范围决定是否运行 macOS 套件的汇总 job。
