...
struct QuotaMonitorApp: App {
    ...
    init() {
        ...
        // Closing the Sparkle update window must let the app fall back to
        // menu-bar-only mode when no other app window remains (the update
        // window is an AppKit NSWindow outside the SwiftUI scenes, so the
        // scene-side demotion hooks never see it close).
