...
| OSLog / structured logging | ✅ Rust `tracing` | ✅ `OSLog` plus opt-in persistent Developer Mode log. Structured events use normal info/warn/error levels and categories include appserver/importer/poller/pricing/query/scan/settings/storage/ui/uninstall/export. | parity+ |
...
| Keychain access policy for Claude OAuth | n/a | ✅ automatic file-first credential refresh with a non-interactive Keychain fallback; Settings → Advanced only surfaces a recovery warning/button if a saved file-only mode disables Keychain reads | new |
